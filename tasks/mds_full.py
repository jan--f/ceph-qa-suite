...
class TestClusterFull(CephFSTestCase):
    ...
    def test_barrier(self):
        ...
        self.assertEqual(self.fs.mds_asok(["status"])['osdmap_epoch_barrier'], initial_osd_epoch)

        # Sync up clients with initial MDS OSD map barrier
        self.mount_a.open_no_data("foo")
        self.mount_b.open_no_data("bar")

        self.assertEqual(self.mount_a.get_osd_epoch()[0], initial_osd_epoch)
        self.assertEqual(self.mount_b.get_osd_epoch()[0], initial_osd_epoch)
        ...
        self.assertEqual(mount_a_epoch, initial_osd_epoch)
